...
GetOptions( \%hCmdLineOption, 'outdir|o=s', 'infile|i=s', 'splitbam|b=s',
...
my (
    $mergebam_file, $bam_file,  $mapstat_list, $mapstat_file,
    $f1,            $path,      $sf1,          $sPath,
    $key,           $pipeline1, $pipeline2
);
my %bam;
my (
...
$mapstat_list =~
  s/bam_merge\/$pipeline1.*_list\/bam\_merge\.file\.list/samtools\_alignment\_stats\/$pipeline2\/samtools_alignment_stats\.mapstats\.list/;

open( $mergebam_file, "<$hCmdLineOption{'infile'}" )
...
while (<$mergebam_file>) {
    ...
    if ( !( exists $bam{$prefix} ) ) {
        open( $bam_file, "<$hCmdLineOption{'splitbam'}" )
          or die "Error! Cannot open split bam list file";
        while (<$bam_file>) {
            chomp;
            ( $sf1, $sPath, $sPrefix ) = File::Spec->splitpath($_);
            $sPrefix =~ s/_\d_\d_dequence.*//;

            if ( $prefix eq $sPrefix ) {
