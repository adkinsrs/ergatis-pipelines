...
=head1 SYNOPSIS
    ...
    expression_plots.pl    --i <infile> --a <analysis> [--lfc] [--fdr]   
                           [--gene_col] [--rpkm_col] [--read_col] [--o outdir] 
                           [--out_prefix] [--rs r_script] [--rb R_bin_dir] [v]
                           [--rp R parameters]
...
=head1 OPTIONS
    ...
    --read_col [read col]  = For rpkm analysis. Default 7.
...
GetOptions( \%hCmdLineOption,
	    ...
	    'gene_col|g=s','rpkm_col|r=s','read_col|r=s','verbose|v','debug',
            'r_script|rs=s','out_prefix=s','help',
...
if( $hCmdLineOption{'analysis'} eq 'rpkm' ) {
    ...
    $cmd =  $hCmdLineOption{'r_bin_dir'}."/R".
	    " ".$hCmdLineOption{'r_params'}.
            " --args ".$hCmdLineOption{'infile'}." ".$hCmdLineOption{'gene_col'}." ".$hCmdLineOption{'rpkm_col'}." ".$sOutDir." ".$hCmdLineOption{'out_prefix'}." rpkm";
    ...
    $cmd =  $hCmdLineOption{'r_bin_dir'}."/R".
	    " ".$hCmdLineOption{'r_params'}.
            " --args ".$hCmdLineOption{'infile'}." ".$hCmdLineOption{'gene_col'}." ".$hCmdLineOption{'read_col'}." ".$sOutDir." ".$hCmdLineOption{'out_prefix'}." read_count";
...
sub check_parameters {
	...
	if (! (defined $phOptions->{'r_script'})) {
	    $phOptions->{'r_script'} = $RealBin."/rpkm_density_plot.r";
	}
	...
	if (! (defined $phOptions->{'read_col'})) {
	    $phOptions->{'read_col'} = 7 ;
	}
    }
    if (! (defined $phOptions->{'r_script'})) {
	$phOptions->{'r_script'} = $RealBin."/expression_plots.R";
    }
