...
=head1 OPTIONS
    ...
    --l <list_SAM/CXB_alignment_files> = /path/to/listfile of SAM or CXBalignment file(s) for each sample.
...
use List::MoreUtils qw(all);  # 'all' is a function in List::Util >v1.33
...
while (<$fpLST>) {
	...
	# Ensure all files from one line are the same (either all SAM/BAM or all CXB)
	test_same_extension(\@aFiles);

	# Each comma-separated file is the same type so take a representative file.
	push @test_suffix, $aFiles[0] if scalar @aFiles;

	# Variable also takes in CXB files
	$sSamFiles = undef;
...
# Ensure all files from all lines are the same (either all SAM/BAM or all CXB)
test_same_extension(\@test_suffix);
...
sub test_same_extension {
	my $files_arr = shift;
	my $suffix;
    # Ensure all files from all lines are the same (either all SAM/BAM or all CXB)
    if (scalar @$files_arr > 1 ) {
	    ($_, $_, $suffix) = fileparse($$files_arr[0], qr/\.[^.]*/);
	    die "ERROR! Files in list must either be all SAM/BAM or all CXB.\n" unless  all { $_ =~ /$suffix$/ } @$files_arr;
    }
    return;
}
