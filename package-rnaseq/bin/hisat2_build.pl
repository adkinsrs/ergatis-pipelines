...
hisat2_build.pl - script to create HISAT2 index for a reference FastA file

=head1 SYNOPSIS

    hisat2_build.pl --r reference_fasta_file [--o outdir] [--b hisat2_bin_dir] 
...
=head1 OPTIONS
    ...
    --b <hisat2_bin_dir>           = /path/to/hisat2_build binary. Optional. [/usr/local/bin]

    --p <index_prefix>             = prefix for index files. Optional. [ref]

    --a <other_parameters>         = additonal hisat2_build parameters. Optional

    --v                            = generate runtime messages. Optional

=head1 DESCRIPTION

The script executes the hisat2_build script from the HISAT2 software package.
...
GetOptions( \%hCmdLineOption,
            ...
            'hisat2_bin_dir|b=s', 'args|a=s', 'verbose|v',
            'debug',
            'help',
            'man') or pod2usage(2);
...
($bDebug || $bVerbose) ? 
	print STDERR "\nGenerating HISAT2 index for $hCmdLineOption{'reffile'} ...\n" : ();

$sCmd = "ln -sf ".$hCmdLineOption{'reffile'}." ".$sOutDir."/".$hCmdLineOption{'prefix'}.".fa";

exec_command($sCmd);

$sCmd  = $hCmdLineOption{'hisat2_bin_dir'}."/hisat2-build";
...
sub check_parameters {
    ...
    $phOptions->{'hisat2_bin_dir'} = BIN_DIR if (! (defined $phOptions->{'hisat2_bin_dir'}) );
}
