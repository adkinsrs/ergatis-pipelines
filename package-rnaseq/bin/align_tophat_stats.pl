...
GetOptions( \%hCmdLineOption, 'outdir|o=s', 'infile|i=s', 'verbose|v', 'debug',
    'help', 'man' )
  or pod2usage(2);
...
my ( $sOutDir, $prefix );
my ( $tophat_file, $mapstat_list, $mapstat_file, $f1, $path, $key, $pipeline1,
    $pipeline2 );
my %bam;
my (
    $cfile,     $mfile,    $lfile, $rfile,    $pfile,
    $readcount, @arr,      @arr1,  $p_paired, $left_count,
    $tot_reads, $p_mapped, $fout,  $out_all
);
my $right_count = 0;
...
$mapstat_list =~
  s/tophat\/$pipeline1\/tophat\.bam\.list/samtools\_alignment\_stats\/$pipeline2\/samtools\_alignment\_stats\.mapstats\.list/;

open( $tophat_file, "<$hCmdLineOption{'infile'}" )
  or die "Error! Cannot open the tophat list file";
open( $mapstat_file, "<$mapstat_list" )
  or die "Error! Cannot open the mapstat list file";
...
open( $out_all, ">$sOutDir/All_Samples.txt" )
  or die "Error Cannot open output file";

foreach $key ( keys(%bam) ) {
    open( $cfile, "<$bam{$key}{'count'}" )
      or die "Error! Cannot open read count file";
    open( $mfile, "<$bam{$key}{'mapstats'}" )
      or die "Error! Cannot open mapstats file";

    if ( exists( $bam{$key}{"prep"} ) ) {
        open( $pfile, "<$bam{$key}{'prep'}" )
          or die "Error! Cannot open prep info file";
    }
    if ( exists( $bam{$key}{"left"} ) ) {
        open( $lfile, "<$bam{$key}{'left'}" )
          or die "Error! Cannot open left info file";
    }
    if ( exists( $bam{$key}{"right"} ) ) {
        open( $rfile, "<$bam{$key}{'right'}" )
          or die "Error! Cannot open right info file";
    }
    ...
    print $fout
      "\#Total.reads\tMapped.reads\tPercent.Mapped\tPercent.Properly.Paired\n";
