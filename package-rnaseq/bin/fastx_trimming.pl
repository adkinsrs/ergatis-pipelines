...
=head1 SYNOPSIS
    ...
    fastx_trimming.pl --i fastx file [--o outdir] [--b fastx_bin_dir] 
                      ...
                      [--l last_base] [--a other_parameters] [--qs] [--z] [--v]
...
=head1 OPTIONS
    ...
    --a <other_parameters> = additonal fastx parameters. Optional. Refer Fastx Toolkit manual.
...
GetOptions( \%hCmdLineOption,
            ...
            'args|a=s', 'qual_stats|qs', 'gzip|z', 'verbose|v',
            'debug',
            'help',
            'man') or pod2usage(2);
...
sub trim_length {
	...
	($bDebug || $bVerbose) ? 
		print STDERR "\tMedian of Median Per Base Quality Scores\t: $nMedianOfMedians\n" : ();
	
	($bDebug || $bVerbose) ? 
		print STDERR "\tPer Base Quality Score Cut-off\t: ".($nMedianOfMedians - $nDeviation)."\n" : ();
	
	$nTrimLength = $nCount;
	for ($nI = $#aMedQuals; $nI >= 0; $nI--) {
		last if ($aMedQuals[$nI] >= ($nMedianOfMedians - $nDeviation));
		$nTrimLength = $nI;
	}
