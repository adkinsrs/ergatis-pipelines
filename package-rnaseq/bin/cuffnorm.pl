...
cuffnorm.pl - script to generate Cuffnorm transcript normalized expression for Cuffquant results.

=head1 SYNOPSIS

    cuffnorm.pl --i transcripts.gtf --l list_of_SAM_alignment_files [--o outdir] 
...
=head1 OPTIONS
    ...
    --args <other_params>          = additional Cuffnorm parameters. Optional. Refer Cufflinks manual.

    --v                            = generate runtime messages. Optional

=head1 DESCRIPTION

The script executes the cuffnorm script from the Cufflinks RNA-Seq Analysis package
...
GetOptions( \%hCmdLineOption,
            ...
            'labels|L=s', 'num-threads|p=i', 'library-type=s', 
...
($bDebug || $bVerbose) ? 
	print STDERR "\nExecuting Cuffnorm normerential Isoform Expression Analysis for input $hCmdLineOption{'listfile'} ...\n" : ();
...
($bDebug || $bVerbose) ? 
	print STDERR "\nInitiating Cuffnorm Analysis ...\n" : ();

$sCmd  = $hCmdLineOption{'cufflinks_bin_dir'}."/cuffnorm".
		 " -o ".$sOutDir;
...
($bDebug || $bVerbose) ? 
	print STDERR "\nRenaming Cuffnorm output files ...\n" : ();

($_, $_, $sPrefix) = File::Spec->splitpath($hCmdLineOption{'listfile'});

$sPrefix =~ s/.list$//;

#rename_files($sOutDir, ".norm", $sPrefix);
