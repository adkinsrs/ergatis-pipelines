...
while (<$fh>) {
    ...
    $cuff_file =~ s/.isoform_exp.diff//;

    @arr = split(/vs/, $cuff_file) ;
    $cuff_dir = $sOutDir."/raw_data/".$cuff_file ;
    if (! -e $cuff_dir) {
    ...
    while ($files = readdir($dh)) {
		if (! ($files =~ /$cuff_file/)){
			$old_file = $temp."/".$files;
			$new_file = $cuff_dir."/".$files;
		}
		else {
			$new_file = $files;
			$new_file =~ s/.*vs.*.sample.//;
			$old_file = $temp."/".$files;
...
sub check_parameters {
	...
	$phOptions->{'r_script'} = $RealBin."/cummerbund_isoform_analysis.R";
    }
