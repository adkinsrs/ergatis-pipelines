...
my ($filehandle, $key, $fout, $file, $ffile);
...
#for split pipeline
$path = $hCmdLineOption{'outrep'}."/align_tophat_split_stats/".$hCmdLineOption{'pipeline'}."_tophat_stats/align_tophat_split_stats.txt.list";
if( -e $path ) { 
...
foreach $key (keys %sample) {
	...
	foreach $a (@{$sample{$key}{'percent'}}) {
	    ...
	    $prefix =~ s/.Percent.txt// ;
	    $prefix =~ s/.bowtie.sorted_by_name//;
	    while (<$file>) {
		chomp($_);
 ...
 if (exists $sample{$key}{'rpkm'}) {
	foreach $a (@{$sample{$key}{'rpkm'}}) {
	    ...
	    $prefix =~ s/.bowtie.sorted_by_position.genic.coverage.stats.rpkm.txt//; 
	    while (<$file>) {
		chomp($_);
		if ($_ =~ /^\#/) {
