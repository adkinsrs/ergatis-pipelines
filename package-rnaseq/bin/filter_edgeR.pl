...
filter_edgeR.pl -   filter edgeR output on [FDR, log fold change, Read Count, P value]

=head1 SYNOPSIS

 USAGE: filter_on_fc.pl
	[REQUIRED]
	--edgeR_list = list file of .txt output files from edgeR
...
my %worksheets;
my $map_info = {};
my ($file,$IN, $fh, $SA, $out, $summary_file, $summary_all, $i, $p, $stub, $sample_name, $sample1, $sample2, $total_output, $line, $workbook, $cmd, $t, $FDR_col);
...
open( $IN, "<", $options{edgeR_list} ) or die "Error cannot open the list file";
@results_files = <$IN>;
close $IN;
...
foreach $file (@results_files) {
	...
	while (<$fh>) {
		...
		$f = run_filter_checks(\@vals,\%param, $FDR_col);
		if ($f == 0) {
		    $out = $files[$i];
		    find_up_or_down_regulated(\@vals, $out, \%param, \$up[$i], \$down[$i], $FDR_col);
		}
...
sub find_up_or_down_regulated {
	my ($vals, $total_out, $param, $up_reg, $down_reg, $FDR_col) = @_;
	my $i;
	my $l = 2;
	my $data_out = "";
	my $LFC_col = $FDR_col - 2;
	...
	if( $$vals[$LFC_col] > $param->{'UFC'} ) {
	    print $total_out $data_out;
	    $$up_reg ++;
	}

	if( $$vals[$LFC_col] < $param->{'DFC'} ) {
	    print  $total_out $data_out;
	    $$down_reg ++;
	}
...
sub run_filter_checks {
	...
	my $LFC_col = $FDR_col - 2;
	my $p_col = $FDR_col - 1;
	my $last_sample_col = $FDR_col - 3;

	my @rcs ;
	for (my $i = 1; $i <= $last_sample_col; $i++){
		push(@rcs, $$vals[$i]);
	}
		
        # FDR cutoff not satisfied
	if (exists $param->{'FDR'}) {
	    if( $param->{'FDR'} != 0 && $$vals[$FDR_col] > $param{'FDR'} ) {
		$filtered = 1;
	    }
	}
	if (exists $param->{'P'}) {
	    if ($$vals[$p_col] > $param->{'P'} ) {
		$filtered = 1;
	    }
	}
     
#comment out, not like deseq, no inf number in edgeR result  
...
sub check_parameters {
	...
	my @required = qw( edgeR_list output_dir project_name );
	for my $option ( @required ) {
        unless  ( defined $options{$option} ) {
            die "--$option is a required option";
        }
	if (! defined $options{'filters'} ){
	    $options{'filters'} = "FDR=0.05,RC=10,UFC=1,DFC=-1:FDR=0.01,RC=10,UFC=1,DFC=-1:FDR=0.1,RC=10,UFC=1,DFC=-1:P=0.05,RC=10,UFC=1,DFC=-1:P=0.01,RC=10,UFC=1,DFC=-1";
	}
    }
