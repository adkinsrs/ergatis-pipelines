...
open OUT_LIST, ">$bam_list"
  or print_log_msg( $ERROR, "Cannot open $bam_list for writing: $!" );
...
sub is_bam_truncated {
    ...
    $cmd =
      ...
      . $out;
    print_log_msg( $DEBUG,
        "INFO : $sub_name :: Checking to see if BAM file is truncated\nINFO : $sub_name :: Command : $cmd"
    );
    ...
    if ( $exit_code != 0 ) {
        ...
        $cmd =
          ...
          . $out;
        print_log_msg( $DEBUG,
            "INFO : $sub_name :: This version of Samtools does not have 'quickcheck' command.  Using alternate command\nINFO : $sub_name :: Command : $cmd"
        );
        $exit_code = system($cmd);
        if ( $exit_code != 0 ) {
            print_log_msg( $ERROR,
...
# SAdkins - 7/7/16  the filter_dups_lc_seqs component will now output multiple BAM files since it is iterative. Because of this I need to merge the BAM files from the list into a single BAM file.
            if ( scalar @list_files > 1 && $list_files[0] =~ /\.bam$/ ) {
                print_log_msg( $DEBUG,
                    "Found multiple BAM files in list.  Merging into single BAM input"
                );
...
# Ensure the extension does not include .gz so that the correct extensions are processed
                ...
                my ( $base, $dir_path, $ext ) =
                  fileparse( $file_no_gz, qr/\.[^.]*/ );
...
sub check_parameters {
    ...
    if ( defined $opts->{'input_file'} && !-s $opts->{'input_file'} ) {
        print_log_msg( $ERROR,
                "ERROR : $sub_name :: Input file or list "
              . $opts->{'input_file'}
              . " appears to be empty" )
          unless $opts->{'input_file'} =~ /\.pair$/;
    }
